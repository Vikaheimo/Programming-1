...
150178798 ja 150286761
Petrus Pakarinen ja Vili Ikaheimo
petrus.pakarinen@tuni.fi ja vili.ikaheimo@tuni.fi
"""
...
def add_friendship(network, name1, name2):
    ...
    # lopettaa toiminnan jos nimet ovat samat
    if name1 == name2:
        return

    # Lisätään network dictin sisällä oleviin listohin
    # tiedot ystävyyksistä. Jos listaa ei vielä ole
    # luodaan se exceptin avulla
    try:
        if
...
def print_command(network, namelist):
    ...
    if len(namelist) != 0:
        ...
        return
    # Käy kaverilistaa läpi aakkosjärjestyksessä ja tulostaa jokaisen
    # henkilön nimen ja sen jälkeen henkilön kaverit
    for names in sorted(network):
        print(names)
        for friend in sorted(network[names]):
            print(f"- {friend}")
...
def friends_command(network, namelist):
    ...
    if not name.isalpha():
        print(f"Error: {name} is not a valid name.")
        return
    # Tarkistaa onko nimi olemassa
    if name not in network:
        print(f"Error: '{name}' is an unknown name.")
    # Tulostaa olemassa olevan henkilön kaverit aakkosjärjestyksessä
    else:
        print(name)
        for f in sorted(network[name]):
            print(f"- {network[name[f]]}")
...
def common_friends_command(network, namelist):
    ...
    for name in namelist:
        if not name.isalpha():
            print(f"Error: {name} is not a valid name.")
            return
    # Tarkistaa onko nimi nimilistassa. Jos ei ole
    # tulostaa error-viestin ja lopettaa suorituksen
    for name in namelist:
        if name not in network:
            print(f"Error: '{name}' is an unknown name.")
    ...
    name1 = namelist[0]
    name2 = namelist[1]
    # Tarkistaa että samalla henkilöllä ei ole samoja kavereita itsensä kanssa
    if name1==name2:
    ...
    for friend in network[name1]:
        if friend in network[name2] and friend not in common_friends:
