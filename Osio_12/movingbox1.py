...
A Program for making and managing moving boxes.
"""
...
class MovingBox:
    ...
    def __init__(self, name):
        """
        Laatikko-objekti luodaan antamalla sille nimi. Luodaan myös items sanakirja johon tallennetaan
        kaikki laatikon tavarat muodossa:
        tavaran_nimi:tavaran_kappalemäärä

        :param name: str, laatikon nimi
        """
        self.__name = name
        self.__items = {}
    ...
    def add_item(self, item_name, item_count):
        """
        Metodi tavaroiden lisäämiseen laatikko-objektiin
        :param item_name: str, lisättävän tavaran nimi
        :param item_count: int, lisättävän tavaran kappalemäärä
        :return: None
        """
        # Jos tavara löytyy jo items-sanakirjasta, lisätään vain kappalemäärä sen arvoon
        if item_name in self.__items:
            self.__items[item_name] += item_count

        # Jos tavara ei vielä ole items-sanakirjassa, niin luodaan sinne uusi key-value pari, jossa
        # avaimena on tavaran nimi ja arvona tavaran kappalemäärä
        else:
            self.__items[item_name] = item_count

    def list_content(self):
        """
        Metodi laatikon sisällön tulostamiseen näytölle.
        :return: None
        """
        # Tulostaa näytölle laatikon nimen, ja kuinka monta tavaraa laatikossa on
        print(f"Box \"{self.__name}\" contains {sum(self.__items.values())} items.")

        # Käydään items-sanakirja läpi ja tulostetaan jokaisen tavaran
        # kappalemäärä ja nimi omalle rivilleen
        for item in sorted(self.__items):
            print(f"{self.__items[item]:3} {item}")

    def remove_item(self, item_name, item_count):
        """
        Metodi tavaroiden poistamiseen laatikko-objektista
        :param item_name: str, lisättävän tavaran nimi
        :param item_count: int, lisättävän tavaran kappalemäärä
        :return: Bool, jos tavaran poistaminen onnistuu palauttaa True, muutoin palauttaa False
        """
        # Tarkisetaan jos tavaraa ei ole laatikossa, niin palautetaan suoraan False
        if item_name not in self.__items:
            return False

        # Jos poistettavien tavaroiden määrä (item_count argumentti) on yhtäsuurta kuin items-sanakirjassa
        # olevan arvo, niin poistetaan tavara kokonaan items-sanakirjasta
        elif self.__items[item_name] == item_count:
            del self.__items[item_name]
            return True

        # Jos taas poistettavien tavaroiden määrä (item_count argumentti) on pienempää kuin items-sanakirjassa
        #  oleva arvo, niin poistetaan arvosta item count arvon verran
        elif self.__items[item_name] > item_count:
            self.__items[item_name] -= item_count
            return True

        # Jos poistettavia tavaroita (item_count) on enemmän kuin laatikossa olevia tavaroita
        # kuin items-sanakirjassa oleva arvo, niin palautetaan False, koska tavaroiden poistaminen
        # ei ole mahdollista
        else:
            return False
    ...
    def transfer_item(self, target_box, item_name, item_count):
        """
        Metodi tavaroiden siirtämiseen laatikosta toiseen
        :param target_box: Movingbox objekti, laatikko johon tavarat halutaan siirtää
        :param item_name: str, siirrettävän tavaran nimi
        :param item_count: int, siirrettävän tavaran kappalemäärä
        :return: Bool / None, Palauttaa arvon True jos tavaroiden siirtäminen onnistuu, muutoin palauttaa None
        """
        # Tarkistetaanm jos tavara ei ole laatikossa, jolloin palaamme
        if item_name not in self.__items:
            return

        # Tarkistetaan jos tavaroita on liian vähän siirtämiseen, jolloin palaamme
        elif self.__items[item_name] < item_count:
            return
        # Muutoin tavaraoita on oikea määrä siirtämiseen
        else:
            # Poistetaan ensin tavara alkuperäisestä laatikosta
            self.remove_item(item_name, item_count)

            # Lisätään sitten tavara haluttuun laatikkon
            target_box.add_item(item_name, item_count)

            # koska tavaroiden siirtäminen onnistui palautetaan True
            return True

    def search_item(self, item_name):
        """
        Metodi tavaran kappalemäärän etsimiseksi laatikosta
        :param item_name: str, etsittävän tavaran nimi
        :return: int, palauttaa etsittävän tavaran kappalemäärän, jos tavaraa ei ole laatikossa
                 niin palautetaan arvo 0.
        """
        if item_name not in self.__items:
            return 0
        else:
